/**
 * plugin-html-button-space-hold-retry
 *
 * Extends plugin-html-button-hold-retry with spacebar support. Participants
 * can hold down the physical spacebar key as an alternative to clicking and
 * holding the on-screen button. While the spacebar is held the targeted button
 * receives the same "jspsych-btn-held" visual treatment as a mouse-held press.
 *
 * New parameter vs. the original plugin:
 *   spacebar_choice  (INT, default 0)
 *     Index into `choices` of the button that the spacebar controls.
 *
 * All other behaviour (min_hold_duration, retries, data fields, etc.) is
 * identical to plugin-html-button-hold-retry.
 *
 * Compatible with jsPsych v8.x
 *
 * Usage:
 *   <script src="plugin-html-button-space-hold-retry.js"></script>
 *
 *   var trial = {
 *     type: jsPsychHtmlButtonSpaceHoldResponse,
 *     stimulus: '<p>Hold the button (or spacebar) as long as you feel confident.</p>',
 *     choices: ['Respond'],
 *     spacebar_choice: 0,       // spacebar targets choices[0]
 *     min_hold_duration: 0,
 *     retries_allowed: 2,
 *   };
 */

var jsPsychHtmlButtonSpaceHoldResponse = (function (jspsych) {
  "use strict";

  const info = {
    name: "html-button-space-hold-response",
    version: "1.0.0",
    parameters: {
       ...
       * The scalar data fields always reflect the final accepted attempt.
       */
      retries_allowed: {
        type: jspsych.ParameterType.INT,
        default: null,
      ...
      /**
       * Index into `choices` of the button that the spacebar controls.
       * When the spacebar is held down, this button receives the visual
       * "held" treatment and the hold is measured identically to a mouse hold.
       */
      spacebar_choice: {
        type: jspsych.ParameterType.INT,
        default: 0,
    ...
    data: {
      ...
      /** Time from trial start to press on the final accepted attempt (ms). */
      press_time: { type: jspsych.ParameterType.INT },
      /** Time from trial start to release on the final accepted attempt (ms). */
  ...
  class HtmlButtonSpaceHoldResponsePlugin {
    constructor(jsPsych) {
    ...
    trial(display_element, trial) {
      ...
      styleEl.textContent = `
        .jspsych-html-button-space-hold-response-button .jspsych-btn {
          font-size: 1.2em;
        }
        .jspsych-btn-held {
      ...
      let html = `<div id="jspsych-html-button-space-hold-response-stimulus">${trial.stimulus}</div>`;
      html += `<div id="jspsych-html-button-space-hold-response-btngroup" style="${layoutStyle}">`;
      for (let i = 0; i < trial.choices.length; i++) {
        html += `<div class="jspsych-html-button-space-hold-response-button"
                      data-choice="${i}"
      ...
      let spacebarDown      = false; // guard against key-repeat fires
      ...
      const getButtonEl = (index) => {
        const wrapper = display_element.querySelector(
          `.jspsych-html-button-space-hold-response-button[data-choice="${index}"]`
        );
        return wrapper
      ...
      const setHoldButtonsDisabled = (disabled) => {
        display_element
          .querySelectorAll(".jspsych-html-button-space-hold-response-button button")
          .forEach((b) =>
      ...
      const endTrial = () => {
        ...
        display_element
          .querySelectorAll(".jspsych-html-button-space-hold-response-button")
          .forEach((wrapper) => {
            const btn =
              wrapper.querySelector("button, [role=button]") ?? wrapper.firstElementChild;
            if (btn) {
      ...
      const onMouseDown = (e) => {
        const wrapper = e.currentTarget.closest(".jspsych-html-button-space-hold-response-button");
        if (!wrapper) return;
        e.preventDefault();
      ...
      if (trial.stimulus_duration !== null) {
        this.jsPsych.pluginAPI.setTimeout(() => {
          const stimEl = display_element.querySelector(
            "#jspsych-html-button-space-hold-response-stimulus"
          );
  ...
  HtmlButtonSpaceHoldResponsePlugin.info = info;
  return HtmlButtonSpaceHoldResponsePlugin;
})(jsPsychModule);
